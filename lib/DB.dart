...
class MyDatabase {
  ...
  Future<List<Map>> getTask(String taskName) async {
    var db = await initDb();
    return db
        .query(tableName, where: "$columnTaskName=?", whereArgs: [tableName]);
  }
  ...
  Future<List<Map>> getTaskWhere(bool value) async {
    ...
    var db = await initDb();
    return db
        .query(tableName, where: "$columnTaskValueName=?", whereArgs: [_value]);
  }

  Future updateTask(TaskModel taskModel) async {
    var db = await initDb();
    db.update(tableName, TaskModel.toMap(taskModel),
        where: "$columnIdName=${taskModel.id}");
  }
